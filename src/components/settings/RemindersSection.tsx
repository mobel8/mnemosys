/**
 * Settings → "Rappels" — absorbed from the former `/planner` page.
 *
 * CRUD over « si [heure] alors j'étudie » study intentions. Time plans are
 * armed as **OS-level notifications** (see `src/lib/notifications.ts`; the
 * re-arming itself happens globally in `<PlannerNotificationsShell>` whenever
 * the plan list changes, so this section only needs to mutate the list).
 *
 * Simplification vs the old page: the form only offers the « time » trigger.
 * The legacy « place » / « after_habit » trigger types never fired anything —
 * they were read-only commitment notes — so they are gone from the form.
 * Existing plans of those types are still listed (toggle/delete work) with an
 * « Informatif » badge, but can no longer be edited into existence.
 */
...
export function RemindersSection() {
  ...
  // --- form state ---
  // `editingId` is `null` in create mode, or the id of the time plan being
  // edited. Only « time » plans are editable — the form has no other trigger.
  ...
  /** Load an existing time plan into the form for editing. */
  ...
  function onSubmit(e: FormEvent) {
    ...
    if (triggerValue.trim().length === 0) {
      toast({ title: "Renseigne l'heure du rappel.", variant: "destructive" });
      return;
    }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Programme des intentions « à telle heure, j'étudie » : une notification système te le
          rappelle au moment choisi, même si l'application est fermée.
        </CardDescription>
      </CardHeader>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              <Label htmlFor="trigger-value">Quand ? (heure)</Label>
              <Input
                id="trigger-value"
                type="time"
                value={triggerValue}
          ...
          <div className="flex items-center gap-2">
            <Button type="submit" disabled={submitting} className="gap-2">
              {isEditing ? (
                ...
                <>
                  <Plus className="h-4 w-4" /> Ajouter le rappel
                </>
              )}
            </Button>
            {isEditing && (
          ...
          {plans.isLoading ? (
            ...
            <div className="flex flex-col items-center gap-3 rounded-xl border border-dashed bg-muted/30 px-6 py-10 text-center">
              ...
              <p className="max-w-sm text-sm text-muted-foreground">
                Choisis une heure et l'action de révision associée dans le formulaire ci-dessus.
              </p>
            </div>
          ) : (
            <ul className="space-y-2">
              {rows.map((plan) => {
                const isTimePlan = plan.trigger_type === "time";
                return (
                  <li
                    key={plan.id}
                  ...
                  >
                    ...
                    {/* Legacy place/habit plans never fire a notification — they
                        are kept visible as read-only commitments. */}
                    {!isTimePlan && (
                      <Badge variant="secondary" title="Ce rappel n'envoie pas de notification.">
                        Informatif
                      </Badge>
                    )}
                    <Switch
