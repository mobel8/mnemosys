 ...
 * `/decks/abc/new-card` used to parse to `NaN`, which silently propagated
 * through the IPC layer: every query gated on `Number.isFinite(deckId)` simply
 * never ran, so the page hung on an eternal skeleton instead of failing
 * cleanly. We now reject non-integer / non-positive ids up front with
